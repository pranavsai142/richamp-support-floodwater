...
#       Add UTC time marker if not existing in cold start date
    ...
    for index in range(timesteps):
        time = coldStartDate + timedelta(seconds=float(timeData[index].data) * 60)
        times.append(time.timestamp())

    print("start of data (seconds since coldstart)")
    startDate = coldStartDate + timedelta(seconds=float(minT) * 60)
    endDate = coldStartDate + timedelta(seconds=float(maxT) * 60)
